...
export class ManageGiftComponent implements OnInit {
  ...
  constructor(public modalRef: MDBModalRef, private fb: FormBuilder, private adminGiftService: AdminGiftService) { }
  ...
  ngOnInit() {
    ...
    this.manageForm = this.fb.group({
      giftName: [this.content ? this.content.giftName : '', Validators.required],
      description: [this.content ? this.content.description : '', Validators.required],
      count: [this.content ? this.content.count : '', Validators.required],
      cost: [this.content ? this.content.cost : '', Validators.required],
      discount: [this.content ? this.content.discount : ''],
      categoryId: [this.content ? this.content.categoryId : '-Category-', this.categoryValidator],
      brandId: [this.content ? this.content.brandId : '-Brand-', this.brandValidator],
    ...
    if (this.adminGiftService.categories.length < 1) {
      this.adminGiftService.getCategories().subscribe((categories: Category[]) => {
  ...
  onSubmit() {
    ...
    if (this.heading === 'Edit Gift') {
      this.adminGiftService.updateGift({key: this.content.key, giftedCount: this.content.giftedCount ? this.content.giftedCount : null,
  ...
  categoryValidator(control: AbstractControl): { [key: string]: boolean } | null {
